...
function StandardCharacterDetail({ characterId }: { characterId: string }) {
  ...
  const movieEntries = useMemo(() => {
    ...
    const charEntriesNorm = new Set((character.entries || []).map(normalize));
    ...
    return MCU.filter((m) => {
      ...
      // 1. Direct match with character.entries
      const isDirectMatch =
        charEntriesNorm.has(mIdNorm) ||
        charEntriesNorm.has(mTitleNorm) ||
        (character.entries || []).some((e) => {
          const eNorm = normalize(e);
          return (
            eNorm === mIdNorm ||
            (eNorm === "avengers" && mIdNorm === "theavengers") ||
            (eNorm === "antmanquantumania" && (mIdNorm === "antmanandthewaspquantumania" || mIdNorm === "antmanquantumania")) ||
            (eNorm === "infinitywar" && (mIdNorm === "avengersinfinitywar" || mIdNorm === "infinitywar")) ||
            (eNorm === "endgame" && (mIdNorm === "avengersendgame" || mIdNorm === "endgame")) ||
            (eNorm === "guardiansvol3" && mIdNorm === "guardiansofthegalaxyvol3") ||
            (eNorm === "guardiansholiday" && (mIdNorm === "theguardiansofthegalaxyholidayspecial" || mIdNorm === "guardiansholiday")) ||
            (eNorm === "capcivilwar" && mIdNorm === "captainamericacivilwar") ||
            (eNorm === "capwintersoldier" && mIdNorm === "captainamericathewintersoldier") ||
            (eNorm === "capfirstavenger" && mIdNorm === "captainamericathefirstavenger")
          );
        });

      // 2. Exact match in the movie's characters list
      const isCharacterInMovie = (m.characters || []).some(
        (c) => normalize(c) === charIdNorm || c === character.id
      );

      return isDirectMatch || isCharacterInMovie;
    });
  }, [character]);

  const isCurrentVillain = isVillainCharacter(character);

  const roleCharacterPool = useMemo(() => {
    return CHARACTERS.filter((c) => isVillainCharacter(c) === isCurrentVillain);
  }, [isCurrentVillain]);

  const currentIndex = roleCharacterPool.findIndex((c) => c.id === character.id);
  const prevCharacter = currentIndex > 0 ? roleCharacterPool[currentIndex - 1] : roleCharacterPool[roleCharacterPool.length - 1];
  const nextCharacter = currentIndex < roleCharacterPool.length - 1 ? roleCharacterPool[currentIndex + 1] : roleCharacterPool[0];
      ...
      <footer className="relative z-10 w-full max-w-4xl px-4 sm:px-12 md:px-16 py-12 sm:py-16 flex flex-col gap-6 items-start">
        ...
        <div className="w-full flex items-center justify-between gap-4">
          ...
          >
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[8px] sm:text-[9px] font-mono uppercase tracking-widest text-stone-500 truncate">
                {isCurrentVillain ? "PREVIOUS VILLAIN" : "PREVIOUS HERO"}
              </span>
          ...
          >
            <div className="flex flex-col min-w-0">
              <span className="text-[8px] sm:text-[9px] font-mono uppercase tracking-widest text-stone-500 truncate">
                {isCurrentVillain ? "NEXT VILLAIN" : "NEXT HERO"}
              </span>
